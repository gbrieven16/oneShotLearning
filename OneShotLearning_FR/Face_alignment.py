...
# ---------------- # TODO ---------------
# Adapt so that it can align jpeg images
...
def image_align(src_file, dst_file, face_landmarks, output_size=1024, transform_size=4096, enable_padding=False):
    ...
    if ZIP_FILE is not None:
        with zipfile.ZipFile(ZIP_FILE, 'r') as archive:
            img = PIL.Image.open(BytesIO(archive.read(src_file)))
    else:
...
if __name__ == "__main__":
    ...
    # ------------------------------------
    # CASE 1: TEST: on decompressed folder
    # ------------------------------------
    if case_id == 1:
        # ---- Go over each image --------
        for img_name in os.listdir(RAW_IMAGES_DIR):
            raw_img_path = os.path.join(RAW_IMAGES_DIR, img_name)
            print("raw_img_path Before loop2: " + raw_img_path)
            # ---- Go over each "landmarks" (i.e detected faces) --------
    ...
    if case_id == 2:
        ...
        for i, fn in enumerate(file_names):  # fn = name[!]_id.extension
            for i, face_landmarks in enumerate(landmarks_detector.get_landmarks(fn), start=1):
                face_img_name = '%s_%02d.png' % (os.path.splitext(fn)[0], i)
                aligned_face_path = os.path.join(ALIGNED_IMAGES_DIR, face_img_name)

                image_align(fn, aligned_face_path, face_landmarks)
