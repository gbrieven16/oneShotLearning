...
"""
IN: images_list: list of the name of image files
    img size: size of resulting image 
    zip_file: name of the zip containing the images in the list (if there's any)
"""
def load_images(images_list, img_size, zip_file):
    loaded_images = list()
    for img_fname in images_list:
        ...
        if zip_file is not None:
            ...
            img = image.load_img(img_fname, target_size=(img_size, img_size))
...
class PerceptualModel:
    ...
    def set_reference_images(self, images_list, zip_file=None):
        assert(len(images_list) != 0 and len(images_list) <= self.batch_size)
        loaded_image = load_images(images_list, self.img_size, zip_file)
